...
class ResidualMagnitudeLayer(nn.Module):
...
def residual_magnitude_loss_fn(pred, target, params):
...
def residual_magnitude_nested_sets_from_output(model, output, lam=None):
  if lam == None:
      if model.lhat == None:
